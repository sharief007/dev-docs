...
{{< details title="Your job scheduler uses at-least-once delivery with lease-based retries. A 'send welcome email' job fails mid-execution, gets retried, and sends the email twice. The product team says 'make it exactly-once.' Can you?" closed="true" >}}
**You can't achieve exactly-once delivery — you achieve exactly-once processing.** The job will be delivered at least once (that's the lease guarantee). Exactly-once *effect* comes from the consumer being idempotent:

1. Before sending the email, check a `job_done:{job_id}` key in Redis. If it exists, skip.
2. After sending the email, set the key with a TTL (e.g., 7 days).

The subtle trap: if the worker sends the email and crashes before setting the `job_done` key, the retry sends again. To truly deduplicate: use the email provider's idempotency key (most support this). The email provider deduplicates even if your system retries.
{{< /details >}}

{{< details title="Your distributed cron uses a Redis SETNX leader lock with a 30-second TTL. The leader process hangs (GC pause, network stall) for 35 seconds. The lock expires, a new leader is elected, and now both nodes think they're the leader. How do you prevent double-firing?" closed="true" >}}
**You can't prevent two nodes from *thinking* they're the leader — you prevent two *firings* of the same job.** The cron scheduler's `cron_fired:{job_name}:{time_bucket}` deduplication key is the real protection. Even if two leaders try to enqueue the same cron job, the `SETNX` on the dedup key ensures only one succeeds.

**The deeper fix:** Use a fencing token. When acquiring the lock, get a monotonically increasing token. When enqueuing a job, include the token. The queue rejects enqueue attempts with a stale token. This is the same pattern as Redlock's fencing — the lock itself isn't sufficient for correctness; the fencing token is.
{{< /details >}}

{{< details title="Tenant A has 100K queued jobs and Tenant B has 10. Your weighted round-robin scheduler gives each tenant equal share. But Tenant A is on the Enterprise plan paying 100x more. Should they get priority?" closed="true" >}}
**Yes — but priority and fairness are different knobs.** The correct approach is **weighted** round-robin where the weight reflects the SLA tier:

- Enterprise (Tenant A): weight 10 → gets 10 jobs dequeued per round
- Free tier (Tenant B): weight 1 → gets 1 job dequeued per round

Tenant B still makes progress (never fully starved), but Tenant A gets 10× the throughput. Additionally, set a **per-tenant concurrency cap** — even Enterprise tenants shouldn't consume all workers. The combination of weighted scheduling + concurrency caps gives proportional throughput without starvation.

The anti-pattern: giving Enterprise tenants *strict* priority (always serve A before B). This starves free-tier users entirely during load spikes, causing churn.
{{< /details >}}
