...
{{< details title="You deploy a new version of your WebSocket server. Even with graceful draining (GoAway message + 60-second grace period), 5% of clients don't reconnect within the window. They silently lose their connections and miss messages until they manually refresh. What's going wrong?" closed="true" >}}
**The clients aren't handling the GoAway message.** Several failure modes:

1. **Mobile apps in background.** The OS suspends the app; the GoAway frame arrives but the app can't process it. The connection dies when the server force-closes. Fix: mobile clients must implement reconnection on `onDisconnect`, not just on `GoAway`.
2. **Corporate proxies.** Some proxies don't forward WebSocket control frames. The client never sees the GoAway. Fix: send an application-level JSON message (`{"type": "server_draining", "reconnect_after": 5}`) *before* the GoAway.
3. **Missing exponential backoff.** Clients that do reconnect but all hit the same new server simultaneously can cause connection failures. Fix: jitter in reconnection timing.

**The deeper fix:** clients should *always* assume the connection can drop at any time. The GoAway is a courtesy, not a guarantee.
{{< /details >}}

{{< details title="Your system has 10M concurrent WebSocket connections across 100 servers. You use Redis Pub/Sub with per-server channels for cross-server routing. A viral group chat has 50K members spread across all 100 servers. User A sends a message. How many Redis Pub/Sub messages are needed?" closed="true" >}}
**100 messages — one per server channel, not 50K per user.** The message is published to each server's channel that has at least one group member connected. With 50K members spread across 100 servers, every server has members, so all 100 server channels receive the message.

Each server then locally delivers to its connected group members (fan-out from 1 pub/sub message to ~500 local WebSocket pushes per server). The pub/sub layer handles O(servers) messages, not O(members).

**The bottleneck:** If this group generates 100 messages/second, that's 10K Redis Pub/Sub messages/second just for this one group (100 messages × 100 servers). At scale, hot group chats can saturate Redis Pub/Sub. Mitigation: for large groups, use a dedicated Kafka topic per group — each server consumes from the partition, getting persistent and ordered delivery.
{{< /details >}}

{{< details title="You calculate: 10M connections × 20KB per connection = 200GB of memory across 100 servers. But your monitoring shows actual memory usage is 400GB — 2× your estimate. Where's the extra memory going?" closed="true" >}}
**The 20KB estimate covers only the TCP kernel buffers. Application-level state doubles it:**

1. **Per-connection application state:** user ID, session metadata, subscription lists, message queue buffers — easily another 10-20KB per connection.
2. **TLS state:** If using WSS (TLS), each connection holds a TLS session object with symmetric keys, certificates, and buffers — ~20-50KB per connection.
3. **Framework overhead:** Go's goroutine stack (2-8KB per goroutine), Node.js event emitter objects, Java NIO buffers — runtime-specific overhead per connection.
4. **Send/receive buffers:** OS-level TCP buffers are tunable but default to 16KB+ per direction per socket.

**Rule of thumb:** budget 40-80KB per WebSocket connection in production (not 20KB). For 10M connections, expect 400-800GB total. This is why WebSocket servers target 100K-500K connections, not 1M — you hit memory limits before file descriptor limits.
{{< /details >}}
