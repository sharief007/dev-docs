...
{{< details title="Your IoT platform uses QoS 2 for all messages because 'we can't afford to lose anything.' Each sensor publishes every 10 seconds, and you have 500K sensors. The 4-packet handshake per message is causing broker CPU saturation. How do you fix this without losing data?" closed="true" >}}
**Switch to QoS 1 with idempotent consumers.** QoS 2's 4-packet handshake (PUBLISH → PUBREC → PUBREL → PUBCOMP) is 4× the network overhead of QoS 1's 2-packet exchange (PUBLISH + PUBACK). At 500K sensors × 0.1 msg/s = 50K messages/second, that's 200K packets/s for QoS 2 vs 100K for QoS 1.

Include a unique `event_id` in each payload. The consumer deduplicates using a Redis SET with TTL: `SETNX dedup:{event_id} 1 EX 60`. This gives you effectively-once processing at QoS 1 cost. Reserve QoS 2 only for actuator commands (unlock door, dispense medication) where the broker itself must guarantee no duplicate delivery.
{{< /details >}}

{{< details title="A temperature sensor publishes to 'factory/line-3/temp' every 5 seconds. A monitoring dashboard subscribes 2 hours later. Without retained messages, the dashboard shows nothing until the next publish (up to 5 seconds). With retained messages, it shows the last known value immediately. But what if the sensor died 30 minutes ago — the retained message shows a stale value with no indication it's old. How do you handle this?" closed="true" >}}
**Include a timestamp in the payload, not just the value.** The retained message becomes `{"temp": 72.5, "ts": 1714041000}`. The dashboard checks: if `now - ts > 2 × publish_interval`, display the value as stale (grayed out) or show "sensor offline."

**Better approach: combine retained messages with LWT.** The sensor's LWT publishes `{"status": "offline"}` with `retain=true` to `factory/line-3/status`. The dashboard subscribes to both `factory/line-3/temp` and `factory/line-3/status`. If the status retained message says "offline," the dashboard knows the temperature value is stale regardless of its age.
{{< /details >}}

{{< details title="A colleague proposes using HTTP POST for IoT sensor telemetry instead of MQTT: 'HTTP is simpler, every device supports it, and we can use REST APIs.' The sensors run on ESP32 microcontrollers with 520KB RAM on cellular networks. Why is this a bad idea?" closed="true" >}}
**Three concrete problems:**

1. **Connection overhead.** Each HTTP request requires a TCP handshake (1 RTT) + TLS handshake (2 RTTs) + HTTP headers (~200-500 bytes). MQTT's persistent TCP connection amortizes the handshake across all messages. On a high-latency cellular network (200ms RTT), HTTP adds 600ms per request vs MQTT's 2-byte header on an existing connection.

2. **Battery life.** The TLS handshake alone consumes significant power on constrained devices. MQTT's keepalive ping (2 bytes every 30-60 seconds) uses orders of magnitude less energy than establishing a new HTTPS connection per reading.

3. **No built-in QoS, offline queuing, or LWT.** With HTTP, you must implement retry logic, delivery tracking, offline message buffering, and disconnect detection in every device's firmware. MQTT provides all of these at the protocol level.

HTTP is fine for devices with ample resources (phones, laptops) on reliable networks. For constrained devices on cellular/satellite, MQTT wins decisively.
{{< /details >}}
