...
{{< details title="A celebrity with 30M followers posts 10 times per day. With fan-out on write, that's 300M inbox writes per day from one user. Your system uses the hybrid approach (celebrities use fan-on-read). But the celebrity's management team complains: 'Our posts take 3 seconds longer to appear in followers' feeds compared to regular users.' Why, and is this acceptable?" closed="true" >}}
**The 3-second delay is because of the fan-out-on-read merge step.** For celebrity content, the feed is computed at read time: the reader's app fetches the inbox (instant) plus queries recent posts from each celebrity they follow (extra latency). The merge/sort adds 50-200ms per celebrity query. If a reader follows 15 celebrities, that's 1-3 seconds of additional read latency.

**This is acceptable.** The alternative — 300M synchronous writes per celebrity post — would delay *all* users' write paths and could take minutes, not seconds. The 3-second read-side penalty affects only the moment a reader opens their feed. A common optimization: pre-compute and cache the merged celebrity feed per reader on a schedule (every 30 seconds), so the latency is amortized.
{{< /details >}}

{{< details title="Your notification pipeline uses Redis SETNX for deduplication with a 1-hour TTL. A user likes a post, unlikes it, then likes it again 45 minutes later. The second like is suppressed as a duplicate. Is this correct behavior?" closed="true" >}}
**It depends on the dedup key design.** If the key is `user_123:like:post_456`, then yes — the second like within the TTL window is treated as a duplicate. This is usually **correct behavior** for notifications: the recipient doesn't need to see "X liked your post" twice in an hour for the same post.

**If you want to allow re-notifications:** include a monotonic action ID in the dedup key: `user_123:like:post_456:action_789`. Each like/unlike/re-like gets a unique action ID, so re-likes generate new notifications. Most social platforms choose the simpler approach (suppress within window) because users find repeated like/unlike notifications annoying.
{{< /details >}}

{{< details title="You send a push notification via APNs (iOS). APNs returns HTTP 200, meaning Apple accepted it. But the user claims they never received the notification. What happened?" closed="true" >}}
**APNs HTTP 200 means Apple's server accepted the message — not that it was delivered to the device.** Several things can prevent delivery:

1. **Device is offline.** APNs stores one notification per app and delivers when the device reconnects. If you send 5 notifications while offline, the user sees only the last one.
2. **User disabled notifications** for your app in iOS Settings. APNs still accepts the message but never displays it.
3. **Token is valid but stale.** The device token works, but the user uninstalled and reinstalled the app. The old token routes to nowhere.
4. **Notification was coalesced.** If you set the same `apns-collapse-id` on multiple notifications, only the latest is shown.

**Key lesson:** Push notifications are **best-effort delivery**. Never rely on push as the sole notification channel. Always store notifications in the in-app inbox as the source of truth, and use push as a supplementary alert.
{{< /details >}}
