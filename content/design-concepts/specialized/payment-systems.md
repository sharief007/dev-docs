...
{{< details title="A user taps 'Pay' and your server calls Stripe. Stripe charges the card and returns success, but your server crashes before recording the result. The outbox consumer retries and calls Stripe again with the same idempotency key. Stripe returns the original result (success). But what if Stripe's idempotency key has expired (Stripe expires them after 24 hours)?" closed="true" >}}
**If 24 hours pass between the crash and the retry, Stripe treats it as a new charge — the user is double-charged.** This is why you need multiple layers of defense:

1. **Recovery job with short SLA.** Scan for payments stuck in `PROCESSING` for > 5 minutes (not 24 hours). Query Stripe by your payment ID to check if the charge exists. If yes, record the result. If no, retry with a fresh idempotency key.
2. **Nightly reconciliation** as the safety net. Compare your ledger against Stripe's records. Any charge in Stripe without a matching internal record is a `MISSING_INTERNAL` discrepancy — a P0 alert.

The lesson: idempotency keys have TTLs. Your recovery window must be shorter than the processor's idempotency TTL.
{{< /details >}}

{{< details title="Your payment service writes a payment record and an outbox event in one DB transaction. The outbox consumer reads the event and calls Stripe. But the consumer crashes after calling Stripe and before committing its Kafka offset. On restart, it re-reads the event and calls Stripe again. The idempotency key prevents a double charge. But now Stripe returns 'already processed' — how does the consumer know if the original call succeeded or failed?" closed="true" >}}
**Stripe returns the full original response for idempotent retries — including the success/failure status.** If the original charge succeeded, the retry returns the same success response with the charge ID. If the original charge was declined, the retry returns the same decline response.

The consumer processes the response identically regardless of whether it's the first attempt or a retry. This is why idempotency keys are so powerful: they turn a "did my previous call succeed?" problem into a simple "call again and see what happens" solution. The key insight: **idempotent retries are not just safe, they're self-healing.**
{{< /details >}}

{{< details title="Your nightly reconciliation finds 3 charges in Stripe that don't exist in your ledger. The discrepancy alert pages the on-call engineer. What's the correct remediation process — refund automatically or investigate first?" closed="true" >}}
**Never auto-refund — always investigate first.** The 3 missing-from-ledger charges could have multiple root causes:

1. **Race condition:** The payment is still in-flight (`PROCESSING` state). The reconciliation ran before the recovery job finished. Action: wait and re-check.
2. **Genuine crash gap:** The server crashed between Stripe charge and ledger write. Action: backfill the ledger entry from Stripe's charge data, deliver the product/service.
3. **Bug:** Application code called Stripe without going through the payment state machine. Action: fix the bug, backfill the ledger.

Auto-refunding case #2 would mean the user paid, you refund, and they got nothing — you lose both the revenue and the customer's trust. The reconciliation alert should create a ticket with full details (Stripe charge ID, amount, timestamp, customer), and a human determines the correct action.
{{< /details >}}

{{< details title="You store monetary amounts as integers in cents (4999 for $49.99). A European customer pays €49.99 and a Japanese customer pays ¥4999. The Japanese yen has no decimal subunit. Your system multiplies all amounts by 100 to convert to 'cents.' What goes wrong?" closed="true" >}}
**¥4999 becomes 499900 — a 100× overcharge.** Not all currencies have 2 decimal places:

- USD, EUR, GBP: 2 decimal places (cents, pence)
- JPY, KRW: 0 decimal places (no subunit)
- BHD, KWD: 3 decimal places (fils)

**Solution:** Store amounts as the smallest currency unit *for that currency*. Use a currency-aware multiplier: `{"USD": 100, "JPY": 1, "BHD": 1000}`. Stripe's API handles this correctly — `amount: 4999` in JPY means ¥4999, while `amount: 4999` in USD means $49.99. Always validate the currency's exponent from a trusted source (ISO 4217) before any arithmetic.
{{< /details >}}
