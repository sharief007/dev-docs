...
{{< details title="Your Snowflake generator throws ClockMovedBackwardError during a routine NTP sync that adjusts the clock back by 200ms. Your service is generating 2,000 IDs/second and crashes. Is refusing to generate IDs the right behavior?" closed="true" >}}
**For 200ms, no — waiting is better than crashing.** The implementation should distinguish small skew (< 5ms: spin-wait) from large skew (> 5s: refuse and alert). A 200ms backward jump means the generator should spin-wait 200ms, then resume. This costs at most 200ms × 2000 IDs/s = 400 IDs delayed — trivial.

**Why refusing is dangerous:** If you throw an exception, every request that needs an ID fails. A 200ms NTP correction is routine and expected. The real danger is large clock jumps (minutes), which indicate VM migration or hardware issues. Implement a tiered response: wait for small skew, refuse for large skew, and always log/alert.
{{< /details >}}

{{< details title="Your team wants to migrate from UUID v4 to UUID v7 (time-ordered) for primary keys. UUID v7 is 128-bit like v4 but has a 48-bit timestamp prefix. A colleague says 'we get Snowflake benefits with no machine ID coordination.' What's the catch?" closed="true" >}}
**UUID v7 solves the B-tree fragmentation problem** (timestamp prefix makes inserts roughly sequential), but there are two catches:

1. **Still 128 bits.** Twice the size of Snowflake's 64 bits. Indexes are larger, joins are slower, and you can't fit them in a JavaScript `Number` (but you couldn't with v4 either).
2. **No structural uniqueness guarantee within a millisecond.** Two servers generating UUIDs in the same millisecond rely on 74 bits of randomness for uniqueness — statistically safe but not structurally guaranteed like Snowflake's machine_id + sequence.

UUID v7 is the right choice when you want drop-in UUID compatibility with sorted inserts and can't manage machine IDs. Snowflake is better when you need 64-bit compactness or structural (not statistical) uniqueness.
{{< /details >}}

{{< details title="Twitter chose 64-bit Snowflake IDs. Why not 128-bit? They had the engineering talent to handle either." closed="true" >}}
**Three concrete reasons for 64-bit:**

1. **JavaScript `Number.MAX_SAFE_INTEGER` is 2⁵³.** Even 64-bit IDs require string serialization in JSON. 128-bit would be worse — no native integer type in most languages handles 128 bits efficiently.
2. **Storage and index efficiency.** Twitter stores billions of tweets. A `BIGINT` primary key is 8 bytes; a UUID is 16 bytes. That doubles index memory for every table and secondary index. At Twitter's scale, this is terabytes of extra RAM.
3. **Throughput.** 4,096 IDs/ms per machine was sufficient for Twitter's tweet volume. They didn't need the larger address space of 128 bits.

The constraint drove a better design. 64-bit forced a disciplined bit layout (timestamp + machine + sequence) that gives you extractable metadata from the ID itself — something UUIDs and ULIDs don't provide as cleanly.
{{< /details >}}
