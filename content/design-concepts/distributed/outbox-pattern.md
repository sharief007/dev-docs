---
title: Outbox Pattern
weight: 10
type: docs
---

Your order service inserts a row into PostgreSQL and then publishes an `OrderCreated` event to Kafka so the inventory service, payment service, and notification service can react. The DB write succeeds, but the Kafka publish times out — and now downstream services have no idea the order exists. Or the Kafka publish succeeds and *then* the DB transaction rolls back due to a constraint violation — and now downstream services are processing an order that doesn't exist. **No amount of retry logic in your application code can fix this**, because the database and the message broker are two independent systems with no shared transaction. The outbox pattern dissolves the problem by writing to only one system — the database — and letting a relay extract events from the database's own log.

The Outbox Pattern solves the **dual-write problem**: how do you atomically update a database **and** publish an event to a message broker? The answer is — you don't. You write both to the database in a single transaction, and a separate process publishes the event later.

## The Dual-Write Problem

A common pattern in event-driven microservices: when business data changes, publish an event so other services can react.

```python
def create_order(order):
    db.save(order)                        # Step 1: write to database
    kafka.publish("OrderCreated", order)  # Step 2: publish to Kafka
```

This code has three failure modes, and **all of them break consistency**:

```mermaid
sequenceDiagram
    participant App as Application
    participant DB as Database
    participant K as Kafka

    Note over App,K: Failure Mode 1: Kafka publish fails

    App->>DB: INSERT order ✓
    App->>K: Publish OrderCreated ✗ (Kafka timeout)
    Note over DB,K: DB has the order<br/>Kafka never got the event<br/>Downstream services don't know

    Note over App,K: Failure Mode 2: App crashes between writes

    App->>DB: INSERT order ✓
    Note over App: 💥 Process crashes
    Note over K: Event never published

    Note over App,K: Failure Mode 3: DB fails after Kafka publish

    App->>K: Publish OrderCreated ✓
    App->>DB: INSERT order ✗ (constraint violation)
    Note over DB,K: Event published for an order that doesn't exist
```

You cannot fix this by reordering the writes. Publishing first means events can exist without data. Writing first means data can exist without events. These are **two independent systems** — no amount of application-level retry logic makes them atomic.

## The Outbox Solution

Instead of publishing directly to [Kafka](../../messaging/kafka), write the event to an **outbox table** in the **same database transaction** as the business data:

```mermaid
sequenceDiagram
    participant App as Application
    participant DB as Database
    participant Relay as Message Relay
    participant K as Kafka

    App->>DB: BEGIN TRANSACTION
    App->>DB: INSERT INTO orders (id, product, amount) VALUES (...)
    App->>DB: INSERT INTO outbox (id, event_type, payload) VALUES (...)
    App->>DB: COMMIT ✓

    Note over DB: Both writes are atomic<br/>Either both exist or neither does

    Relay->>DB: SELECT * FROM outbox WHERE published = false
    DB->>Relay: [{id=1, event_type="OrderCreated", payload="..."}]

    Relay->>K: Publish OrderCreated
    K->>Relay: ACK

    Relay->>DB: UPDATE outbox SET published = true WHERE id = 1
```

**Why this works:** The database's own ACID transaction guarantees that the business data and the outbox row are written atomically. The event publication is decoupled into a separate, eventually-consistent process.

### Outbox Table Schema

```sql
CREATE TABLE outbox (
    id            UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    aggregate_type VARCHAR(255) NOT NULL,    -- "Order", "Payment", "User"
    aggregate_id   VARCHAR(255) NOT NULL,    -- the business entity's ID
    event_type    VARCHAR(255) NOT NULL,     -- "OrderCreated", "OrderCancelled"
    payload       JSONB NOT NULL,            -- the full event data
    created_at    TIMESTAMP DEFAULT NOW(),
    published     BOOLEAN DEFAULT FALSE      -- used by polling relay
);
```

The `aggregate_type` + `aggregate_id` fields enable routing: events for the same aggregate go to the same Kafka partition, preserving ordering within an entity.

## Publishing: Polling vs CDC

### Polling Relay

The simplest approach: a background process periodically queries the outbox table.

```
Every 500ms:
  SELECT * FROM outbox WHERE published = false ORDER BY created_at LIMIT 100;
  For each row:
    Publish to Kafka
    UPDATE outbox SET published = true WHERE id = ?
```

| Pros | Cons |
|------|------|
| Simple to implement | Polling interval adds latency (up to 500ms) |
| Works with any database | Polling queries add load on the database |
| No external dependencies | Requires the `published` flag and cleanup |

### CDC with Debezium (Recommended)

**Change Data Capture** reads the database's internal replication log (PostgreSQL WAL, MySQL binlog) and streams changes to Kafka in near real-time.

```mermaid
sequenceDiagram
    participant App as Application
    participant DB as PostgreSQL
    participant WAL as WAL (Write-Ahead Log)
    participant Deb as Debezium Connector
    participant K as Kafka

    App->>DB: BEGIN; INSERT order; INSERT outbox; COMMIT;
    DB->>WAL: Write outbox INSERT to WAL

    Deb->>WAL: Read new WAL entries (logical replication slot)
    WAL->>Deb: INSERT on outbox table: {event_type="OrderCreated", payload="..."}

    Deb->>K: Publish to topic "outbox.events.Order"
    K->>Deb: ACK

    Deb->>Deb: Advance replication slot LSN
    Note over Deb: Position tracked — no duplicate on restart
```

**Why CDC is better than polling:**

| | Polling | CDC (Debezium) |
|---|---|---|
| **Latency** | Polling interval (100ms–1s) | Near real-time (10–50ms) |
| **Database load** | Periodic SELECT queries | Zero — reads the WAL passively |
| **Published flag** | Needed | Not needed — position tracked in WAL |
| **Delivery guarantee** | At-least-once (could miss on crash) | At-least-once (WAL is the source of truth) |
| **Cleanup** | Must delete old outbox rows | Can delete immediately after commit (Debezium reads WAL, not the table) |

**Debezium outbox routing:** Debezium has a built-in [Outbox Event Router](https://debezium.io/documentation/reference/transformations/outbox-event-router.html) that reads the outbox table structure and routes events to Kafka topics based on `aggregate_type`. No custom code needed.

## At-Least-Once Delivery

The relay (polling or CDC) may crash **after** publishing to Kafka but **before** recording that it published:

```
Relay reads outbox row → publishes to Kafka → 💥 crash before marking published

On restart: reads same row again → publishes to Kafka AGAIN → duplicate event
```

This is inherent to the pattern. The outbox guarantees **at-least-once** delivery, not exactly-once. Consumers **must be [idempotent](../idempotency)**.

## The Inbox Pattern (Consumer-Side Deduplication)

The consumer-side complement to the outbox. Each consumer maintains an **inbox table** to track which events it has already processed:

```mermaid
sequenceDiagram
    participant K as Kafka
    participant C as Consumer
    participant DB as Consumer's Database

    K->>C: OrderCreated (eventId=abc-123)

    C->>DB: BEGIN TRANSACTION
    C->>DB: SELECT id FROM inbox WHERE event_id = 'abc-123'
    Note over DB: Not found — first time seeing this event

    C->>DB: INSERT INTO inbox (event_id) VALUES ('abc-123')
    C->>DB: UPDATE accounts SET balance = balance - 99 WHERE id = 42
    C->>DB: COMMIT ✓

    C->>K: ACK (offset committed)

    Note over K,DB: Duplicate delivery attempt:

    K->>C: OrderCreated (eventId=abc-123) — redelivered

    C->>DB: BEGIN TRANSACTION
    C->>DB: SELECT id FROM inbox WHERE event_id = 'abc-123'
    Note over DB: Found! Already processed.
    C->>DB: ROLLBACK (skip processing)

    C->>K: ACK (idempotent — no double-processing)
```

```sql
CREATE TABLE inbox (
    event_id    VARCHAR(255) PRIMARY KEY,
    processed_at TIMESTAMP DEFAULT NOW()
);

-- Optional: clean up old entries
DELETE FROM inbox WHERE processed_at < NOW() - INTERVAL '7 days';
```

The inbox check and the business logic execute in the **same database transaction** — the same atomic trick as the outbox.

## Complete Pipeline

```mermaid
flowchart LR
    subgraph Producer Service
        A[App Logic] -->|single txn| B[(DB + Outbox)]
    end

    B -->|WAL| C[Debezium CDC]
    C --> D[Kafka Topic]

    subgraph Consumer Service
        D --> E[Consumer]
        E -->|single txn| F[(DB + Inbox)]
    end
```

```
Producer side:  Business write + outbox row in ONE transaction → atomicity
Transport:      Debezium reads WAL → publishes to Kafka → at-least-once
Consumer side:  Inbox dedup + business write in ONE transaction → exactly-once processing
```

The end-to-end guarantee is **effectively exactly-once**: at-least-once delivery (outbox + Debezium) combined with idempotent consumption (inbox) means every event is processed exactly once, even under failures.

## When to Use the Outbox Pattern

| Scenario | Use Outbox? | Why |
|----------|------------|-----|
| Service writes to DB and needs to notify other services | **Yes** | The canonical use case |
| [Saga](../saga-pattern) orchestrator sending commands | **Yes** | Orchestrator's state + outgoing command must be atomic |
| Service writes to DB and updates a cache | **Maybe** | CDC-based cache invalidation is simpler (no outbox table, just read the WAL) |
| Service only publishes events (no DB write) | **No** | No dual-write problem — publish directly |
| Service needs exactly-once Kafka production | **Consider** | Kafka's idempotent producer + transactions may suffice within a single Kafka pipeline |

{{< callout type="info" >}}
**Interview framing:** "The order service writes the order and an outbox event in a single database transaction — atomicity is guaranteed by the database. Debezium tails the WAL and publishes the event to Kafka. Consumers are idempotent using an inbox table. This gives us effectively exactly-once delivery across service boundaries without distributed transactions."
{{< /callout >}}

{{< callout type="info" >}}
**Interview tip:** The outbox pattern is my standard answer to **the dual-write problem** — the moment a service has to update its own DB and publish to Kafka, I write the business row and an outbox row in the same DB transaction so either both happen or neither does, and let a separate relay publish the events. I'd strongly prefer Debezium reading the WAL (CDC) over a polling relay — no `published` flag to manage, no DB load from periodic queries, and near-real-time latency. The honest caveat I'd raise unprompted: this is **at-least-once delivery, not exactly-once** — Debezium can republish events on restart, so consumers must be idempotent (inbox table or business-key dedup in the same transaction as the side effect). Combined, you get effectively exactly-once *processing* across service boundaries without distributed transactions.
{{< /callout >}}

## Test Your Understanding

{{< details title="Why can't you fix the dual-write problem by simply publishing to Kafka first and writing to the DB second — or by wrapping both writes in application-level retry logic?" closed="true" >}}
**Because every ordering leaves a failure window, and retries can't close it.** Publish-first means the process can crash (or the DB write can fail a constraint) *after* the event is out — now there's an event for an order that doesn't exist. DB-first means the process can crash after the commit but before the publish — now there's an order with no event. Reordering just moves the window; it never removes it.

Retries don't help because **the crash can happen between the two operations**, before any retry loop runs — and even if it didn't, the database and the broker share no transaction, so there's no way to make "both committed" an atomic fact. The only fix is to write to **one** system atomically (DB row + outbox row in a single transaction) and derive the publish from that.
{{< /details >}}

{{< details title="The outbox guarantees at-least-once, not exactly-once. Pinpoint the exact instant a duplicate is created — separately for a polling relay and for Debezium CDC." closed="true" >}}
The duplicate is always born in the same structural gap: **"publish to Kafka" and "record that I published" are two steps, and the relay can crash between them.**

- **Polling relay:** it publishes the row to Kafka, then crashes *before* `UPDATE outbox SET published = true`. On restart it re-reads the still-`false` row and publishes again.
- **Debezium:** it publishes the WAL event to Kafka, then crashes *before* committing its advanced offset / LSN. On restart it resumes from the last committed position and re-emits the event.

In both cases the publish itself succeeded but the bookkeeping didn't survive the crash. That's why the guarantee is at-least-once and why **consumers must be idempotent** — there is no way to make the publish and its acknowledgement atomic.
{{< /details >}}

{{< details title="With Debezium CDC there is no `published` boolean column, and you can delete the outbox row in the very same transaction that inserts it. How does the event still get published?" closed="true" >}}
**Because Debezium reads the write-ahead log, not the table.** The instant the `INSERT INTO outbox` is written to the WAL, the event is durably captured — that WAL record exists independently of the row's later fate. You can `INSERT` then `DELETE` the row in the same transaction (a common trick to keep the table empty): the WAL still contains the INSERT, so Debezium streams it; the DELETE just produces a separate change event the router ignores.

There's no `published` flag because progress is tracked by **LSN/offset** in the replication slot, not per-row state. This is the core reason CDC beats polling: zero query load on the table, no flag to maintain, and the outbox table never accumulates rows.
{{< /details >}}

{{< details title="Two events for the same order — OrderCreated then OrderCancelled — are written to the outbox. How do you guarantee consumers observe them in that order, and where can ordering still silently break?" closed="true" >}}
**Partition Kafka by `aggregate_id` (the order ID)** so both events land on the same partition, and Kafka preserves order *within* a partition. The relay must also emit them in `created_at` / WAL order. This is exactly what the `aggregate_type` + `aggregate_id` columns in the schema are for.

Ordering breaks if: you partition by `event_id` or a random key (the two events scatter across partitions, losing their relative order); a **multi-threaded relay** publishes concurrently without preserving per-aggregate ordering; or you expect a *global* order across different orders — Kafka only guarantees per-partition order, and cross-aggregate ordering is neither provided nor usually needed.
{{< /details >}}

{{< details title="In the inbox pattern, why must the INSERT INTO inbox and the business UPDATE run in the SAME transaction? Walk through what breaks if you instead insert the inbox row, commit, and then do the business write." closed="true" >}}
**Same-transaction makes "mark processed" and "apply the effect" atomic — either both happen or neither does.** That's the whole point of the inbox.

If you split them — insert the inbox row, commit, then do the business write — a crash *between* the commit and the business write is catastrophic: the event is now permanently recorded as "processed," but its effect never happened. On redelivery the consumer sees the inbox hit, concludes "already handled," and **skips** — so the update is lost forever, with no error anywhere. Wrapping both in one transaction means a crash rolls back the inbox insert too, so redelivery correctly re-processes the event. The inbox dedup is only safe when it commits together with the work it's vouching for.
{{< /details >}}

{{< details title="Your consumer is idempotent via an inbox table, but its actual job is to send a payment-receipt email through a third-party provider. Does the inbox give you exactly-once email? Why or why not?" closed="true" >}}
**No.** The inbox transaction can only dedupe effects that live in the *same database* — it can roll back the inbox row and a DB write together. An email send is an **external, non-transactional side effect**: it cannot be enrolled in your DB transaction. The consumer can send the email, then crash before committing the inbox row → redelivery → the email is sent a second time.

The honest guarantee for external effects is **at-least-once**. To get closer to once: make the side effect itself idempotent — pass a provider-side **idempotency key** (most email/payment APIs support this) so the provider dedupes — or route the email through its *own* outbox so the send is driven from committed state. "Exactly-once processing" is a statement about state changes in your database, never about arbitrary external actions.
{{< /details >}}

{{< details title="Your Debezium connector is down for 6 hours during an incident. What happens inside PostgreSQL during that window, and why can a stalled CDC consumer threaten the primary database itself?" closed="true" >}}
**The logical replication slot pins the WAL.** PostgreSQL cannot recycle WAL segments beyond the slot's last-confirmed LSN until Debezium consumes them — so for 6 hours of writes, WAL **accumulates on the primary's disk**. If `pg_wal` fills the volume, the primary can refuse writes or crash. A "passive" CDC reader becomes an availability risk for the source database precisely because it holds back WAL cleanup.

Mitigations: monitor **replication slot lag** and alert on connector downtime; set **`max_slot_wal_keep_size`** (PG13+) to cap retained WAL — at the cost that an over-lagged slot gets invalidated and the connector must re-snapshot; and treat a long Debezium outage as a database incident, not just a pipeline one. This is the classic operational gotcha of CDC-based outbox that polling relays don't have.
{{< /details >}}
