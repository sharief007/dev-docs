...
{{< details title="You use RabbitMQ for email sending. Consumer A crashes before sending. What happens to the email?" closed="true" >}}
**Depends on ACK mode.** With manual ACK: the message was never acknowledged. RabbitMQ detects the disconnect and **redelivers** to another consumer. Email is sent once. With auto-ACK: the message was ACK'd on delivery, before processing — it's gone, email never sent.

**Rule:** Always manual ACK for tasks that must complete. ACK after successful processing.
{{< /details >}}

{{< details title="A Kafka topic has 3 consumer groups. A new event is published. How many times is it delivered? How does this differ from RabbitMQ?" closed="true" >}}
**Kafka:** Delivered to **one consumer per group** — 3 deliveries total. The event stays in the log; groups read independently.

**RabbitMQ:** A message on a queue goes to **exactly one consumer**. For fan-out, you need a fanout exchange binding to 3 separate queues — the message is copied 3 times.

Kafka separates storage from consumption (one event, N independent readers). RabbitMQ copies messages per subscriber. This is why Kafka suits event architectures (replay, fan-out) and RabbitMQ suits task distribution (single consumer, DLQ).
{{< /details >}}

{{< details title="A team uses Kafka for task distribution. A consumer crashes after processing but before committing the offset. The email is sent twice. How do you prevent duplicate sends?" closed="true" >}}
You can't prevent duplicate **delivery** (Kafka is at-least-once by default). Prevent the duplicate **side effect** by making the consumer idempotent:

1. Assign each task a unique `task_id`. Check a deduplication store (Redis SETNX) before sending. If present → skip.
2. Use `topic:partition:offset` as a globally unique deduplication key.
3. If the email provider supports idempotency keys (SendGrid, Mailgun), pass the `task_id` — the provider deduplicates server-side.
{{< /details >}}
