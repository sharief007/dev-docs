...
{{< details title="An LSM tree writes are sequential appends. A B+ tree write involves random page updates. Why does this make LSM trees faster for writes?" closed="true" >}}
...
{{< details title="A point read in an LSM tree might check the memtable, then L0, then L1, ... up to LN. With 7 levels, that's 7+ lookups. How do Bloom filters help, and when do they fail?" closed="true" >}}
...
{{< details title="You're choosing between leveled and size-tiered compaction for a Cassandra table. The table receives 500K writes/sec and occasional full-partition reads. Which strategy and why?" closed="true" >}}
