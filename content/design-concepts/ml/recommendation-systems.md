...
{{< details title="Your recommendation system optimizes for click-through rate (CTR) and achieves 15% CTR in A/B testing — up from 10%. But average watch time drops from 8 minutes to 3 minutes, and user retention falls 5% over the next month. What went wrong?" closed="true" >}}
...
{{< details title="A brand-new user opens your app for the first time. They have zero interaction history. Your collaborative filtering model can't generate any signal because it relies on user-item co-occurrence. What do you recommend, and how do you quickly escape the cold start?" closed="true" >}}
...
{{< details title="Your two-tower retrieval model pre-computes item embeddings offline and stores them in a FAISS index. A new item is uploaded at 3 PM. It won't appear in recommendations until the nightly embedding rebuild at 2 AM. How do you reduce this cold-start-for-items latency?" closed="true" >}}
