...
{{< details title="Your colleague argues: 'Why use CDC when we can just have the application publish events to Kafka after each DB write? It's simpler and we control the event schema.' What's the strongest counter-argument?" closed="true" >}}
**The dual-write problem is not hypothetical — it's guaranteed to bite you.** The application writes to the DB, then publishes to Kafka. Three failure modes:

1. DB write succeeds, Kafka publish fails → data in DB, not in downstream systems.
2. Kafka publish succeeds, DB transaction rolls back → phantom event for non-existent data.
3. Both succeed but app crashes before confirming → retry publishes a duplicate.

CDC eliminates all three because it reads from the DB's own transaction log — it sees **exactly** the committed changes. The DB's WAL is the single source of truth; CDC is just tailing it.

**The event schema argument is valid:** CDC events are schema-coupled to the DB table. If you need a different event shape, use CDC to read the outbox table (where you control the event schema) rather than reading application tables directly.
{{< /details >}}

{{< details title="Debezium is streaming CDC events from PostgreSQL to Kafka. The Debezium connector crashes and stays down for 6 hours. During that time, PostgreSQL accumulates WAL segments that it can't reclaim because the replication slot holds them. Your DB disk fills up and the production database stops accepting writes. How do you prevent this?" closed="true" >}}
**Set `max_slot_wal_keep_size` in PostgreSQL (v13+).** This limits how much WAL a replication slot can retain. When the limit is exceeded, PostgreSQL reclaims the oldest WAL segments even if the slot consumer hasn't read them.

**The trade-off:** If Debezium comes back after the WAL was reclaimed, it can't resume from its last position. It must perform a **full re-snapshot** of the tables — which is expensive but recoverable. The alternative (disk full, database down) is catastrophic.

**Production checklist:**
1. Alert on replication slot lag (`pg_wal_lsn_diff`) at 500MB and 1GB thresholds.
2. Set `max_slot_wal_keep_size = 10GB` as a safety valve.
3. Monitor Debezium connector health; auto-restart on failure.
4. If Debezium is permanently decommissioned, **always drop the replication slot** — orphaned slots are the #1 cause of WAL disk-full incidents.
{{< /details >}}

{{< details title="You use the outbox pattern: the app writes business data + outbox event in one transaction. Debezium reads the outbox table. But your app inserts 10,000 orders/second, each generating an outbox event. The outbox table grows unboundedly. How do you manage it?" closed="true" >}}
**Delete outbox rows after Debezium has consumed them.** Two approaches:

1. **Debezium Outbox Event Router with `table.op.invalid.behavior=warn`.** Configure Debezium to capture only INSERT events from the outbox table. After CDC captures the INSERT, a separate cleanup job deletes rows older than N minutes. The DELETE events are ignored by the outbox router.

2. **Truncate on a schedule.** Run `DELETE FROM outbox_events WHERE created_at < now() - interval '1 hour'` every 10 minutes. Since Debezium reads from the WAL (not the table), deleting rows doesn't affect CDC — the INSERT events are already in the WAL.

**Key insight:** The outbox table is a transient staging area, not a permanent store. Kafka is the durable event log. The outbox table should stay small (< 100K rows) at all times.
{{< /details >}}
