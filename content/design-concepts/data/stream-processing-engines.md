...
{{< details title="Your team debates Flink vs Kafka Streams for a new service that enriches order events with customer data from a local database. The service receives 10K events/second from Kafka, looks up customer info, and writes enriched events back to Kafka. An engineer argues 'Flink is the industry standard for stream processing — we should use it.' Is Flink the right choice here?" closed="true" >}}
**Kafka Streams is the better choice for this use case.** The enrichment is a simple map operation (read event → lookup customer → emit enriched event) with no windowing, no complex state, and moderate throughput.

Kafka Streams advantages here:
1. **No cluster to manage.** It's a library embedded in your microservice. Deploy as a normal Kubernetes pod with auto-scaling.
2. **Simpler operations.** No JobManager/TaskManager, no checkpoint configuration, no separate monitoring infrastructure.
3. **Kafka-native.** Consumer groups handle partition assignment and rebalancing automatically.

**Use Flink when:** you need windowed aggregations, complex event processing, large managed state (GB+), or sub-millisecond latency that Kafka Streams' poll-based model can't achieve. For simple event enrichment, Kafka Streams avoids unnecessary operational complexity.
{{< /details >}}

{{< details title="Your Flink job maintains 500GB of state in RocksDB (per-user fraud scores). A checkpoint takes 3 minutes. During checkpoint, throughput drops 40% because RocksDB snapshots compete for disk I/O. How do you fix this without reducing state size?" closed="true" >}}
**Switch to incremental checkpointing.** Full checkpoints copy the entire 500GB state to persistent storage every time. Incremental checkpoints only copy the SST files that changed since the last checkpoint — typically 1-5% of the total state.

**Additional optimizations:**
1. **Use managed memory for RocksDB.** Configure Flink's memory manager to allocate off-heap memory for RocksDB block cache, reducing GC pressure.
2. **Separate checkpoint storage.** Write checkpoints to a different disk (or S3) than RocksDB's working directory to avoid I/O contention.
3. **Tune checkpoint interval.** If 3-minute checkpoints are too frequent, increase the interval (e.g., 5 minutes). The trade-off: longer recovery time after failure (you replay more events from Kafka).

**Key insight:** Checkpoint interval is a trade-off between steady-state throughput and recovery time. Larger state → longer checkpoints → incremental checkpointing becomes essential.
{{< /details >}}

{{< details title="Your Flink job processes payment events and updates account balances in an external PostgreSQL database. You enable Flink's exactly-once checkpointing. After a failure and recovery, you discover some account balances were updated twice. But Flink guarantees exactly-once! What went wrong?" closed="true" >}}
**Flink's exactly-once applies to its internal state, not to external side effects.** When the job writes to PostgreSQL as a side effect of processing, Flink's checkpoint mechanism doesn't control the external write. After recovery, Flink replays events from the last checkpoint offset, and those replayed events trigger duplicate PostgreSQL writes.

**Solutions:**
1. **Idempotent writes.** Use `INSERT ... ON CONFLICT (account_id) DO UPDATE SET balance = excluded.balance`. Since the replayed event produces the same result, the upsert is safe.
2. **Two-phase commit sink.** Implement a custom Flink sink that pre-commits to PostgreSQL during checkpoint barriers and commits only after the checkpoint completes. This gives true end-to-end exactly-once but adds latency.
3. **Write to Kafka first (transactional sink), then consume into PostgreSQL.** Flink’s Kafka sink supports exactly-once via transactions. A separate idempotent consumer writes from Kafka to PostgreSQL.

Option 1 is simplest and most common. Option 2 is the textbook answer but rarely used in practice due to complexity.
{{< /details >}}
