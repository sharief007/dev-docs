...
{{< details title="Your team runs a Lambda architecture: Spark for batch, Flink for streaming, both producing user engagement scores. A bug is discovered in the engagement formula. You fix it in the Spark codebase (Python) and deploy. A week later, you realize the same bug is still in the Flink codebase (Java). Scores have been inconsistent for 7 days. How do you prevent this?" closed="true" >}}
**This is the fundamental argument for Kappa architecture.** With Lambda, every logic change must be applied to two codebases in different languages with different semantics. The fix:

1. **Short-term:** Share the formula as a configuration (e.g., a JSON rule definition) consumed by both Spark and Flink, so changes are applied once.
2. **Long-term:** Migrate to Kappa. Use Flink for both real-time and historical reprocessing. When the formula changes, replay the Kafka log through the updated Flink job. One codebase, one language, one deployment.

The 2× engineering cost of Lambda is not just initial development — it's every bug fix, every formula change, and every feature addition forever.
{{< /details >}}

{{< details title="Flink claims 'exactly-once' processing. But your Flink job reads from Kafka, updates a counter, and writes to PostgreSQL. After a crash and recovery, you notice the PostgreSQL row was written twice. How is this 'exactly-once'?" closed="true" >}}
**Flink's exactly-once guarantee applies to its internal state, not to arbitrary external sinks.** The checkpoint mechanism (Chandy-Lamport) ensures that Flink's internal counters, windows, and state stores are consistent after recovery. But writing to PostgreSQL is a side effect outside Flink's control.

**For true end-to-end exactly-once, the sink must support transactions.** Flink's two-phase commit sink (e.g., Kafka sink with transactions) pre-commits output during checkpoint and commits only after checkpoint completes. For PostgreSQL, you need an idempotent upsert (`INSERT ... ON CONFLICT DO UPDATE`) using the event's unique key, so replayed events overwrite rather than duplicate.

**Lesson:** "Exactly-once" in stream processing means exactly-once *state update*, not exactly-once *side effect*. Sinks must be either transactional or idempotent.
{{< /details >}}

{{< details title="Your analytics dashboard uses a nightly Spark batch job. The product team wants 'real-time analytics.' You propose replacing Spark with Flink. Your senior engineer pushes back: 'Batch is actually better for this use case.' When is batch genuinely better than streaming?" closed="true" >}}
**Batch is better when:**

1. **The consumers don't need freshness.** If the dashboard is viewed once daily at 9 AM, processing at 2 AM is perfectly fine. Streaming adds 24/7 infrastructure cost for no user-visible benefit.
2. **The computation requires global context.** ML training, percentile calculations (P99 latency), and graph algorithms need the full dataset. Streaming approximations (HyperLogLog, T-Digest) sacrifice accuracy.
3. **Cost optimization matters.** A batch job runs for 2 hours on spot instances at $0.03/core-hour. A Flink cluster runs 24/7 at $0.10/core-hour. For the same daily compute, batch costs 1/12th.
4. **Correctness is non-negotiable.** Financial reporting, regulatory compliance, and billing must be exact. Batch reprocessing is trivially reproducible; stream reprocessing requires careful Kafka offset management.

**Rule of thumb:** If the data consumer can tolerate hours of latency, batch is simpler, cheaper, and more correct. Streaming earns its complexity only when seconds matter.
{{< /details >}}
