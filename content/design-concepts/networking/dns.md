...
{{< callout type="info" >}}
**Interview tip:** When DNS comes up in a system design, lead with TTL discipline: "I'd treat TTL as the most consequential knob — for stable records I'd use 3600s for low query load, but 24–48 hours before any planned migration I'd drop it to 60s so the change actually propagates within a minute. DNS-based failover alone can't guarantee sub-minute recovery because clients keep hitting cached IPs until TTL expiry, so I'd pair short TTLs with application-level retry and circuit breakers. For multi-region routing I'd use GeoDNS with EDNS Client Subnet so routing decisions are based on the actual client IP, not the resolver's IP — otherwise a developer on a US-based VPN routes through US-East and pays 200ms RTT instead of 20ms locally. For the apex domain I'd use Route 53 ALIAS or Cloudflare CNAME flattening because raw CNAMEs at the root violate RFC 1034. And I'd run TXT records for SPF, DKIM, and DMARC — without them, mail from your domain ends up in spam."
{{< /callout >}}
